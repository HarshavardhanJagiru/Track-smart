...
const Login = () => {
    ...
    // Check for verification param from email redirect
    const queryParams = new URLSearchParams(location.search);
    const isRecentlyVerified = queryParams.get('verified') === 'true';
        ...
        <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-700">
            <div className="max-w-md w-full">
                <div className="text-center mb-10 group">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-semibold tracking-wide uppercase text-xs">Log in to your career command center</p>
                </div>

                <div className="card shadow-xl shadow-slate-200/50">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        {successMsg && !error && !isRecentlyVerified && (
                        ...
                        {isRecentlyVerified && !error && (
                            <div className="bg-emerald-50 text-emerald-700 px-4 py-3 rounded-lg text-sm font-medium border border-emerald-200 mb-4 animate-in fade-in slide-in-from-top-1 duration-200 flex items-center gap-2">
                    ...
                    <p className="text-center mt-8 text-slate-600 dark:text-slate-400 font-medium">
                        ...
                        <Link to="/register" className="text-primary-600 dark:text-primary-400 hover:text-primary-700 font-bold">
                            Sign Up
                        </Link>
                    </p>
                </div>
